...
const verifyQueue = async (wbot, msg, ticket, contact) => {
    ...
    if (queues.length === 1) {
        ...
        if (firstQueue.mediaPath !== null) {
            ...
            const filePath = path_1.default.resolve("public", firstQueue.mediaPath);
            const optionsMsg = await (0, SendWhatsAppMedia_1.getMessageOptions)(firstQueue.mediaName, filePath);
    ...
    if (choosenQueue) {
        ...
        if (choosenQueue.options.length === 0) {
            ...
            if (settings?.value === "queue") {
                currentSchedule = await (0, VerifyCurrentSchedule_1.default)(ticket.companyId, queue.id);
            }
            if (settings?.value === "queue" &&
